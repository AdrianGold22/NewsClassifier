...
def initial_data(list_links, file_name):
    file_report = '{0}.csv'.format(file_name)
    data_input = DIR_INPUT + file_report
    np_links = unique_rows(np.array(list_links))
    data = []
    print("\nSources: {0}".format(', '.join(sources)))
    try:
        print('Initial data input in {0} loading {1}...'.format(file_name, len(np_links[:, 0])))
        for page in tqdm(range(len(np_links[:, 0]))):
            ...
            if title is not None:
                ...
                continue
        print('Final data input in {0} loading {1}...'.format(file_name, len(data)))
        print('Data input for {0} save successful!'.format(file_name))
